...
public class SerialHandler {
	...
	HashMap<String, String> value_map = new HashMap<String, String>() {{ put("TEMP", "temperature"); put("LIGT", "light"); put("PH", "ph"); put("EC", "ec"); put("TDS", "tds"); put("FLOW", "flow"); put("LEVL", "level"); put("PUMP", "pump"); }};
	...
	public void handle() throws MqttPersistenceException, MqttException {
		if (port != null) {
			...
			if (port.bytesAvailable() > 0) {
				...
				switch(com) {
				case "V":
					serial_client.publish("value/" + value_map.get(sub_com), new MqttMessage(data.getBytes()));
					break;
				case "S":
					if (res.substring(5, res.length()).equals("1")) {
						serial_client.publish("status/set", new MqttMessage(("{\"name\":\"" + value_map.get(sub_com) + "\",\"value\":\"True\"}").getBytes()));
					} else {
						serial_client.publish("status/set", new MqttMessage(("{\"name\":\"" + value_map.get(sub_com) + "\",\"value\":\"False\"}").getBytes()));
					}
					break;
				case "C":
					switch(sub_com) {
